...
import '../../styles/custom/create_vessel.css';

/**
 * Standalone FORM_ONLY Custom Page Component
 * Form Code: CREATE_VESSEL
 * Destination Path: client/src/pages/custom/create_vessel.jsx
 */
export default function CreateVesselCustomPage() {
  const [schema, setSchema] = useState(null);
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState(null);

  const FORM_CODE = 'CREATE_VESSEL';
  ...
  if (isLoading) {
    return (
      <div className="custom-page-wrapper" style={{ padding: '40px', textAlign: 'center' }}>
        <p>Loading Add Vessel Management Form configuration...</p>
      </div>
    );
  }
